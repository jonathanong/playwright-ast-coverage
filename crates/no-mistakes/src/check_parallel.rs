use crate::check_tasks::{
    run_codebase_check, run_integration_check, run_queue_check, run_react_check, run_rules_check,
    CheckTask,
};
...
pub(crate) fn run_domain_checks(
    ...
    let ((react, queues), (rules, (integration, codebase))) = rayon::join(
        || {
            rayon::join(
                {
    ...
    (react, queues, rules, integration, codebase)
}
